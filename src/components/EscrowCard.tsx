...
import { formatCurrency, truncateAddress } from '@/utils/formatting';
    ...
    <Link href={`/escrow/${escrow.id}`}>
      <div className="bg-white border border-trustflow-border rounded-xl p-6 hover:shadow-md hover:border-blue-300 transition cursor-pointer group">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="font-semibold text-lg text-trustflow-navy">{escrow.title}</h3>
            <p className="text-sm text-slate-500">ID: #{escrow.id}</p>
          </div>
        ...
        <div className="flex justify-between items-end mt-6">
          <div>
            <p className="text-xs text-slate-400 uppercase tracking-wider font-semibold mb-1">Escrow Value</p>
            <p className="text-xl font-bold text-slate-900">{formatCurrency(escrow.amount, escrow.token)}</p>
          </div>
